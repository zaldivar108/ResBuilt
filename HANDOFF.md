...
_Last updated: 2026-07-08 · Branch `master` · Build: green · Tests: 387 passing · npm audit: 0 vulns · v0.2.0 · Prod: https://resbuilt.vercel.app (live, deployed)_

**LATEST (part 10) — two more "Edit my text" tasks: Make concise / Elaborate, UNCOMMITTED:** the per-section AI task row (sidebar `AiControls`) had 4 buttons (Improve wording, Fix grammar, Format, Suggest ideas) — now 6. New `concise` and `elaborate` tasks in `api/ai.js` (both small tier, no JSON mode, maxInput 3500 same as `improve`; `concise` maxTokens 500/temp 0.4 tightens wordy phrasing while keeping every fact; `elaborate` maxTokens 700/temp 0.5 expands thin wording with natural detail, prompt explicitly forbids inventing a new employer/date/number/credential — "if unsure whether a detail is implied, leave it out"). 6 new `api/ai.test.js` cases. Client: `AiInput.jsx` `TASKS`/`MAX_INPUT` extended; `contactBlocked` now also disables Make concise/Elaborate on a contact section (same PII reasoning as Improve/Suggest ideas — Format/Grammar stay, both on-device there); consent copy (workspace + info-orb card) updated to name both. No new lib file needed — both reuse the existing generic `runTask` path verbatim (same privacy model as Improve: full text sent, no PII scrub). Live-verified via `vercel dev`: real 200 from Groq/OpenCode on Make concise, diff preview rendered, budget counter decremented, no console errors. 387/387 tests, lint unchanged (baseline 23), build green.

**LATEST (part 9) — whole-résumé "Improve" button, UNCOMMITTED:** new feature (not one of the tracked gaps #7–9) — a résumé-wide AI action that actually rewrites section content across the whole résumé in one call, not just comments on it (which is all `review`/issue-002 does). New `improveAll` task in `api/ai.js` (large tier, JSON mode, `jsonRequire: 'sections'`, maxInput 8000, maxTokens 2400) + pure `src/lib/resumeImprove.js` (`buildImproveAllPrompt`/`parseImproveAllResult`, 15 new tests) + 5 new `api/ai.test.js` cases. **Privacy note:** unlike `review`, this does **not** scrub PII and does **not** send the contact section at all — same trust model as single-section Improve (see `scrubPii.js`'s own comment on why Improve/Grammar skip redaction).

**Placement (moved once, per user feedback):** first landed as a second block inside the Review modal (`ResumeChecklistPanel.jsx`), then relocated to live alongside the other AI actions — sidebar (`AiControls`) gets a standalone "Improve whole résumé" button (not a per-section mode tab, since its scope is the whole résumé, not the active section); the editor-column workspace (`AiWorkspace`) renders the run/loading/error state and, once results land, a checkbox + `DiffPreview` (word-level ins/del) list per section so the user can exclude any before applying, plus an "Apply N changes" button. Both surfaces share the state via a few new fields on the existing `AiProvider` context (`improveAllState/Result/Error/Selected`, `runImproveAll`, `toggleImproveAllSelected`, `applyImproveAllSelected`) — same sidebar-triggers/workspace-results split every other AI action already uses. `onApplyAll` threads from `Editor.jsx` through `AiProvider` the same way `onApply`/`onApplyRange` already do.

`Editor.jsx`: new `applyAiToAllSections(edits)` batches the checked sections' writes in one `patch`. Generalized `lastAiEdit` from a single `{sectionId, prevContent}` to `{edits: [...]}` so the existing 8s undo toast now supports multi-section undo (`undoAiEdit` restores every touched section together); toast label pluralizes ("AI improved N sections") when more than one section changed.

**Live-verified end-to-end via `vercel dev` + chrome-devtools** (both the original modal placement and the final sidebar/workspace placement): created a full student-starter résumé, ran the improve pass (7 sections came back rewritten, contact correctly excluded), unchecked one section and confirmed the count/diffs updated correctly, applied, confirmed the live preview updated exactly the checked sections, workspace reset to idle after apply. No console errors. 381/381 tests, lint unchanged at baseline 23, build green. **Not yet committed** — review and commit when ready.

**LATEST FIX (part 8) — `599bd2a`, pushed, VERIFIED live 2026-07-08:** user reported "AI couldn't process résumé" on **prod** import. Root cause: `api/ai.js`'s provider-fallback loop had **no per-provider timeout** — OpenCode's free reasoning models (multi-second by nature, worse on the `import` task's larger token budget) could hang past the Edge function's ~25s wall-clock ceiling, so the Groq fallback never got a turn and the whole request died as a 504 instead of degrading. Confirmed live via curl: `import` timed out (504), `improve` (small tier, fast) returned 200 fine. Fix: `AbortController` + timeout per provider attempt (OpenCode 9s, Groq 12s budget) so a hung primary aborts with room left for the fallback inside the edge ceiling. **Re-verified 2026-07-08** via curl against prod: `import` now returns `200` in ~10s with valid `sections[]` JSON — no more 504. Closed. If OpenCode is *routinely* slow on `import` going forward, revisit swapping its large-tier model or dropping OpenCode for that one task (Groq-only) — not needed now.
