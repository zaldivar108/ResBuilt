...
### 2026-07-06
- **AI Assist wired up (free).** `AiInput` dock now functional: paste text → Improve wording / Suggest ideas / Fix grammar. Backed by **Groq free tier** (`llama-3.3-70b-versatile`) via a **Vercel Edge proxy** at `api/ai.js` — the `GROQ_API_KEY` stays server-side (never in the client/bundle). Consent notice shown (text leaves device → Groq); input capped at 2000 chars; client cooldown + loading/error states. **Setup:** create a free key at console.groq.com, set `GROQ_API_KEY` (see `.env.example`); local dev needs `vercel dev` (plain `npm run dev` won't run `/api`). Grammar-only in-browser option (harper.js) still open as a privacy-max alternative.
